# from https://raw.githubusercontent.com/Smerity/sha-rnn/218d748022dbcf32d50bbbb4d151a9b6de3f8bba/model.py
...
class Block(nn.Module):
    def __init__(self, embed_dim, hidden_dim, heads=1, dropout=None, rnn=False, residual=True, use_attn=True, causal=False):
        super().__init__()
        ...
        if rnn:
            if causal: # uni-direction
                num_directions = 1
                self.rnn = nn.LSTM(embed_dim, embed_dim, batch_first=True, bidirectional=False)
            else: # bi-direction
                num_directions = 2
                self.rnn = nn.LSTM(embed_dim, embed_dim, batch_first=True, bidirectional=True)
            
            self.fc = nn.Linear(num_directions*embed_dim, embed_dim)
    ...
    def forward(self, h, pe, attn_mask, mem=None, hidden=None):
        ...
        ## Sam:
        # new_mem only involved in bigh processing, so how this work?
        # Do I need this?
        focus, new_mem = None, []
