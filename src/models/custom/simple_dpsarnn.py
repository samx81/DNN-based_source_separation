...
from models.custom.sharnn import Block as SARNN

EPS=1e-12

class DualPathSARNN(nn.Module):
    def __init__(self, num_features, hidden_channels, num_blocks=6, num_heads=4, norm=True, dropout=0, causal=False, eps=EPS):
        super().__init__()
        ...
        for _ in range(num_blocks):
            net.append(DualPathSARNNBlock(num_features, hidden_channels, num_heads=num_heads, norm=norm, dropout=dropout, causal=causal, eps=eps))
    ...
    def forward(self, input, hidden=None, mems=None):
        """
        Args:
        ...
        for idx, block in enumerate(self.net):
            ...
            h, m, nh, f = block(h, pe, attn_mask=mask, mem=mem, hidden=hid)
            new_hidden += nh
            new_mems += m
...
class DualPathSARNNBlock(nn.Module):
    def __init__(self, num_features, hidden_channels, num_heads=1, norm=True, dropout=0, causal=False, eps=EPS):
        super().__init__()
        
        self.intra_chunk_block = IntraChunkSARNN(
            num_features, hidden_channels, num_heads=num_heads,
            norm=norm, nonlinear=nonlinear, dropout=dropout,
            eps=eps
        )
        self.inter_chunk_block = InterChunkSARNN(
            num_features, hidden_channels, num_heads=num_heads,
            norm=norm, nonlinear=nonlinear, dropout=dropout,
            causal=causal,
            eps=eps
    ...
    def forward(self, h, pe, attn_mask, mem=None, hidden=None):
        ...
        h, m1, nh1, f  = self.intra_chunk_block(h, pe, attn_mask=mask, mem=mem, hidden=hid)
        h, m2, nh2, f  = self.inter_chunk_block(h, pe, attn_mask=mask, mem=mem, hidden=hid)
        
        return h, [nh1, nh2], [m1,m2]


# TODO: Figure out how to feed these features to make SARNN running

class IntraChunkSARNN(nn.Module):
    def __init__(self, num_features, hidden_channels, num_heads=1, norm=True, dropout=0, eps=EPS):
        super().__init__()
        ...
        # nonlinear=nonlinear,
        self.sarnn = SARNN(
            num_features, hidden_channels, heads=num_heads,
            dropout=dropout, rnn=True
        )
        
    def forward(self, input, pe, attn_mask, mem=None, hidden=None):
        """
        Args:
            input (batch_size, num_features, S, chunk_size)
        Returns:
        ...
        x, m, nh, f = self.sarnn(x, pe, attn_mask=attn_mask, mem=mem, hidden=hidden) # -> (chunk_size, batch_size*S, num_features)
        x = x.view(chunk_size, batch_size, S, num_features) # (chunk_size, batch_size*S, num_features) -> (chunk_size, batch_size, S, num_features)
        output = x.permute(1, 3, 2, 0) # (chunk_size, batch_size, S, num_features) -> (batch_size, num_features, S, chunk_size)

        return output, m, nh, f

class InterChunkSARNN(nn.Module):
    def __init__(self, num_features, hidden_channels, num_heads=4, causal=False, norm=True, dropout=0, eps=EPS):
        super().__init__()
...
def _test_transformer_block():
    ...
    model = IntraChunkSARNN(num_features, hidden_channels=hidden_channels, num_heads=num_heads)
    print(model)
    pe = [0]*1000
    mask =None
    output = model(input, pe, mask)
    print(input.size())
    ...
    causal = True
    model = InterChunkSARNN(num_features, hidden_channels=hidden_channels, num_heads=num_heads, causal=causal)
    print(model)
...
def _test_dptransformer():
    ...
    model = DualPathSARNN(num_features, hidden_channels, num_blocks=num_blocks, num_heads=num_heads, causal=causal)
    print(model)
