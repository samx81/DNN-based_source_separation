...
from norm import GlobalLayerNorm

class Complex_STFT(STFT):
    """
    1D Complex STFT
    """
    def __init__(self, bottleneck_channel,  *args, **kwargs):
        super(Complex_STFT, self).__init__(*args, **kwargs)
        self.in_norm = GlobalLayerNorm(512)
        self.in_act = nn.PReLU()
        self.proj = nn.Sequential(
            nn.Conv1d(512, bottleneck_channel, kernel_size=1),
            nn.PReLU()
        )
...
class Complex_ISTFT(ISTFT):
    """
    1D Complex ISTFT
    """
    def __init__(self, bottleneck_channel,*args, **kwargs):
        super(Complex_ISTFT, self).__init__(*args, **kwargs)

        self.proj = nn.Sequential(
            nn.PReLU(),
            nn.Conv1d(bottleneck_channel, 512, kernel_size=1),
