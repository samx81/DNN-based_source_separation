...
parser.add_argument('--criterion', type=str, default='sisdr', choices=['sisdr', 'this_sisdr', 'threshold_snr','demucs_l1','demucs_mse','mse'], help='Criterion')
...
def main(args):
    ...
    print(model1)
    print("# Parameters: {}".format(model1.num_parameters))
    ...
    package = torch.load(args.pretrain_model1, map_location=lambda storage, loc: storage)
            
    if isinstance(model1, nn.DataParallel):
        model1.module.load_state_dict(package['state_dict'])
    else:
        model1.load_state_dict(package['state_dict'])
