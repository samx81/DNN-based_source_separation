   ...
   # loader['train'] = TrainDataLoader(train_dataset, batch_size=args.batch_size,num_workers=16, shuffle=True)
    ...
    if args.handcraft ==2 :
        model = DPTNet_Interact(
            ...
            mask_nonlinear=args.mask_nonlinear, causal=args.causal, handcraft=args.handcraft, batch_size=args.batch_size,
            n_sources=args.n_sources, variation=args.var
        )
    else:
        model = DPTNet(
            args.n_basis, args.kernel_size, stride=args.stride,
